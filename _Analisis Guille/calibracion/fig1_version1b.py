...
sensor_bis_smooth=np.linspace(0,0,n_bis)
...
while i<n_bis:
	h=int(indice_ruido_bis[i])-1
        sensor_bis_smooth[i]=ruido_bis_smooth[h]
	i=i+1
...
ax1.plot(tiempo_bis,sensor_bis*(theta_one-theta_bis),linestyle='-',lw=0.4,color='darkorange',label=r'Ruido del Sensor (para $F=0$)')
ax1.plot(tiempo_bis,sensor_bis_smooth,linestyle='--',lw=0.1,color='steelblue',label=r'Ruido del sensor (mediana)')
...
pylab.savefig('fig1_version1b.eps', format='eps', dpi=300, bbox_inches='tight')
