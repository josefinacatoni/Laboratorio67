...
freqs, times, Sx = signal.spectrogram(sensor, 1.0/0.06)
freqs_bis, times_bis, Sx_bis = signal.spectrogram(sensor_bis, 1.0/0.06)

#ax1.plot(freqs,Sx,lw=0.7,color='darkorange',label=r'Calibraci\'on 1 (cuadrado)')
#ax1.plot(freqs_bis,Sx_bis,lw=0.7,color='steelblue',label=r'Calibraci\'on bis (cuadrado)')

ax1.plot(vernier,sensor,'o',markersize=0.1,color='orange',label=r'Calibraci\'on 1 (cuadrado)')
ax1.plot(vernier_bis,1.2*sensor_bis,'o',markersize=0.1,color='steelblue',label=r'$1.2\times$ Calibraci\'on bis (cuadrado)')

legend = plt.legend(loc='lower right',prop={'size':3})
...
pylab.savefig('fig2_version2b.eps', format='eps', dpi=300, bbox_inches='tight')
