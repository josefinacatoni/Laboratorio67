...
vernier_smooth=np.asarray(sliding_median((vernier_bis-vernier_bis_min)*theta_bis,5))
sensor_smooth=np.asarray(sliding_median(sensor_bis_delta,5))
...
ax1.plot(tiempo_bis,vernier_smooth,lw=0.3,color='firebrick',label=r'Vernier sin el offset (mediana)')
ax1.plot(tiempo_bis,sensor_smooth,lw=0.3,color='darkorange',label=r'Sensor sin el offset (mediana)')
...
np.savetxt('fig1_version1c_maximos.dat', np.transpose([tiempo_bis,vernier_smooth,sensor_smooth]), delimiter='\t', newline='\n')


pylab.savefig('fig1_version1c_sliding.eps', format='eps', dpi=300, bbox_inches='tight')
