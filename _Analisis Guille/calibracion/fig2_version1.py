...
data2= np.genfromtxt('./cuadradoS13.dat', delimiter = '\t') # Tomo la misma data aproposito!!!!!!!!!
...
delta_tiempo=np.linspace(0,0,n_bis)
delta_sensor=np.linspace(0,0,n_bis)
delta_vernier=np.linspace(0,0,n_bis)
sensor_smooth=np.linspace(0,0,n_bis)
vernier_smooth=np.linspace(0,0,n_bis)
...
while i<n_bis:
	delta_tiempo[i-1]=tiempo_bis[i]
	delta_sensor[i-1]=sensor_bis_delta[i]-sensor_bis_delta[i-1]
        delta_vernier[i-1]=(vernier_bis[i]-vernier_bis_min)*theta_bis[i]-(vernier_bis[i-1]-vernier_bis_min)*theta_bis[i-1]
	i=i+1

sensor_smooth=np.asarray(sliding_median(sensor_bis_delta,50))
vernier_smooth=np.asarray(sliding_median((vernier_bis-vernier_bis_min)*theta_bis,50))
...
#ax1.plot(tiempo_bis,indice_bis,lw=0.1,color='gray',linestyle='-',label=r'$\theta(v-v_\mathrm{min})$ ($v_\mathrm{min}=2.58$)')



#ax1.plot(tiempo_bis,2.0*(vernier_bis-vernier_bis_min)*theta_bis,lw=0.3,color='firebrick',label=r'Vernier sin el offset')
#ax1.plot(tiempo_bis,sensor_bis_delta,lw=0.3,color='darkorange',label=r'Sensor sin el offset')

alpha=2.0

ax1.plot(tiempo_bis,sensor_smooth-sensor_bis_delta,lw=0.3,color='darkorange',label=r'Sensor  (sin el offset y sliding mean)')
ax1.plot(tiempo_bis,alpha*(vernier_smooth-(vernier_bis-vernier_bis_min)*theta_bis),lw=0.3,color='firebrick',label=r'Vernier (sin el offset y sliding mean)')
#ax1.plot(vernier_smooth,sensor_smooth,lw=0.1,color='gray',label=r'sima de ambas (sin el offset)')

legend = plt.legend(loc='upper right',prop={'size':2.5})
...
ax1.set_title(r'Cuadrado')


pylab.savefig('fig2_version1.eps', format='eps', dpi=300, bbox_inches='tight')
