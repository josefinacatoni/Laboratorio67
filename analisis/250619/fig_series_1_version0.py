...
histo1_smooth=sliding_median(data1[:,1],200)
histo2_smooth=sliding_median(data1[:,2],200)
histo3_smooth=sliding_median(data1[:,3],200)
histo4_smooth=sliding_median(data1[:,4],200)
...
ax1.plot(time,histo2_smooth,lw=0.7,color='orange',zorder=1,label=r'sensor 2')
ax1.plot(time,histo3_smooth,lw=0.7,color='steelblue',zorder=1,label=r'sensor 3')
ax1.plot(time,histo1_smooth,lw=0.7,color='olive',zorder=1,label=r'sensor 1')

legend = plt.legend(loc='upper right',prop={'size':5})
#ax1.set_ylim(0, 0.005)
ax1.set_xlabel(r'steps')
ax1.set_ylabel(r'Levels')
ax1.set_title(r'Sensores (smoothed with median)')


pylab.savefig('fig_series_1_version0.eps', format='eps', dpi=300, bbox_inches='tight')
