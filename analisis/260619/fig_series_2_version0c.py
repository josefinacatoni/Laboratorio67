...
data1= np.genfromtxt('./26062019_manana.txt', delimiter = '\t')	
...
milestone = np.linspace(0,0,10)
levels    = np.linspace(0,1024,10)
...
ax1.plot(milestone+120,levels,linestyle='--',lw=0.3,color='black',zorder=1,label=r'Ituzaingo')
ax1.plot(milestone+960,levels,linestyle='-',lw=0.4,color='gray',zorder=1,label=r'Haedo')
ax1.plot(milestone+1260,levels,linestyle='--',lw=0.4,color='gray',zorder=1,label=r'Ramos')
ax1.plot(milestone+1680,levels,linestyle='-.',lw=0.4,color='gray',zorder=1,label=r'(contacto espalda)')
ax1.plot(milestone+1980,levels,linestyle=':',lw=0.4,color='gray',zorder=1,label=r'Liniers (linea 28)')
#ax1.plot(milestone+2100,levels,linestyle=':',lw=0.4,color='gray',zorder=1,label=r'Ituzaingo (salida)')
...
legend = plt.legend(loc='upper left',prop={'size':4})
ax1.set_xlim(0, 2500)
ax1.set_ylim(0, 1000)
ax1.set_xlabel(r'time (sec.)')
ax1.set_ylabel(r'Levels')
ax1.set_title(r'Sensores (smoothed with median of $\pm\,$80 sec.)')


pylab.savefig('fig_series_2_version0c.eps', format='eps', dpi=300, bbox_inches='tight')
